...
GameObject::GameObject(GameObject&& other) noexcept 	//이동생성자
{
	...
	m_worldTransform = other.m_worldTransform;
	m_eachTransform = other.m_eachTransform;
	//Matrix4x4::Identity(other.m_worldTransform);
	//Matrix4x4::Identity(other.m_eachTransform);

	m_OOBB = other.m_OOBB;
	m_isOOBBCover = other.m_isOOBBCover;
	//other.m_OOBB = BoundingOrientedBox();
	//other.m_isOOBBCover = false;
	...
	//other.m_pParent.reset();	// other의 부모를 없앤다. 위의 과정을 통해 참조카운트가 0이 되어 other이 소멸될 것이다.
